...
class RegisterUseCaseTest {
    ...
    fun `validateCpf should throw AuthException when first check digit is mathematically invalid`() {
        // CPF com primeiro dígito inválido (o correto para 111.444.777 seria 35)
    ...
    fun `validateCpf should throw AuthException when second check digit is mathematically invalid`() {
        // CPF com segundo dígito inválido (o correto para 111.444.777 seria 35)
    ...
    fun `validateCpf should pass for valid CPFs`() {
        ...
        for (cpf in validCpfs) {
            registerUseCase.validateCpf(cpf) // não deve lançar exceção
        }
    }

    @Test
    fun `registerUseCase should fail with AuthException message when CPF is already registered in Room`() = runTest {
    ...
    @Test
    fun `registerUseCase should succeed for valid CPF and save cryptographic token session`() = runTest {
